...
function checkAnswer() {
    ...
    if (chosen === correct) {
        ...
        // Within-session retry: re-queue the missed word once, but only if there's
        // room to place it a real distance later (>= MIN_GAP other questions in
        // between). Missing a word near the end of a sitting used to make it
        // reappear as the very next card, because the insert clamped to the deck
        // end. When there's no room to space it, skip the in-session retry — the
        // word is already scheduled for review via resetLeitner (next day), so it
        // still comes back "after some time" instead of right away.
        if (state.retryInSession && !wordObj._requeued) {
            const MIN_GAP = 6;
            const room = session.words.length - session.current; // furthest we could insert (append = end)
            if (room >= MIN_GAP) {
                const again = {
                    ...wordObj,
                    _retry: true,
                ...
                const maxGap = Math.min(room, MIN_GAP + 4);
                const gap = MIN_GAP + Math.floor(Math.random() * (maxGap - MIN_GAP + 1));
                session.words.splice(session.current + gap, 0, again);
